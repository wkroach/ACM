...
LL arr[10005];
LL cnt[10005][2];///cnt[i][0]表示i出现的次数, cnt[i][1]表示i中的素因子个数

LL cal(LL n)///O(1)计算C(n,4)而不是用组合数
{
    return n * (n - 1)* (n - 2) * (n - 3) / 24;
}

void solve2()///对当前数计算所有因子，并统计每个因子的素因子数，并计算每个因子在n个数中出现的个数，O(1000)
{
    for(LL i = 1; i < (1 << factors.size()); ++i)
    {
        LL tmp = 1;
...
//        printf("Case #%I64d: ", ++Case);
        ...
        for(LL i = 0; i < n; ++i)///对每个数计算因子，由于因子只在10000以内，所以打表储存每个因子的素因子个数即出现次数
        {///O(n*1000 + n*log(n))
            scanf("%lld",&arr[i]);
            getFactors(arr[i]);
            solve2();
        ...
        for(LL i = 0; i <= 10000; ++i)///对10000内所有数进行计算，如果是这些数的因子，利用容斥，根据素因子的个数进行计算
        {
            if(cnt[i][0])
            {
                if(cnt[i][1] & 1)
        ...
        ans = cal(n) - ans;///求互质的数目，直接计算所有4组合的数相减即可
        printf("%lld\n",ans);
    }
    return 0;
}
