...
//    while(scanf("%I64d%I64d",&n, &m) == 2 && (m + n))
    {
        scanf("%I64d%I64d%I64d%I64d%I64d",&a, &b, &x, &y, &k);
        if(k > max(a, b) || k > max(x, y) || k == 0)
        {
            printf("Case %I64d: ", ++Case);
            cout<<0<<endl;
            continue;
        }
        a--;///注意a，x要先自减再除k，表示a，x以下的结果都不需要
        x--;
        a /= k;
        x /= k;///先除k再自减会产生错误，结果中仍有可能有不需要的结果
        b /= k;
        y /= k;
        LL ans = 0;
        ans = solve(b, y);///1~b,1~y中互质的数目
        LL ans1 = solve(a , y);///1~(a-1),1~y中互质的数目
        LL ans2 = solve(b, x );///1~b,1~(x-1)中互质的数目
        LL ans3 = solve(a, x);///1~(a-1),1~(x-1)中互质的数目
        ans = ans - ans1 - ans2 + ans3;///由于指定了范围，根据容斥原理要减去不需要的结果
//        printf("%lld\n",ans);
        printf("Case %I64d: ", ++Case);
        printf("%I64d\n",ans);
