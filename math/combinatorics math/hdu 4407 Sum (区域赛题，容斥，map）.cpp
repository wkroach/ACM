...
//        printf("Case #%I64d: ", ++Case);
        for(LL i = 0; i < m; ++i)///由于m < 1000因此无需打表（会炸的~），单次计算即可，利用map查询所有更改过的数，O(m^2 * log(m))(好吧我不会算。。。)
        {
            ans = 0;
            LL flag = 0;
            scanf("%I64d",&flag);
            if(flag == 1)
            {
                ...
                getFactors(arr2[3]);///计算质因子
                ans = solveBetween(arr2[1], arr2[2], arr2[3]);///计算初始状态时a,b之间互质数的和
                for(map<LL, LL> :: iterator it = mp.begin(); it != mp.end(); ++it)
                {
                    if(it -> first <= arr2[2] && it -> first >= arr2[1])///如果在a,b范围内有数的修改，单独判断
                    {
                        LL tmp2 = gcd(it -> second, arr2[3]);
                        LL tmp = gcd(it -> first, arr2[3]);
                        if(tmp == 1 && tmp2 != 1)///若初始即为互质数，改后不为，则要减去初始的数
                        {
                            ans -= it -> first;
                        }
                        else if(tmp != 1 && tmp2 == 1)///若初始不互质，改后互质，要加上改后的数
                        {
                            ans += it -> second;
                        }
                        else if(tmp == 1 && tmp2 == 1)///若均为互质数，则要求差值
                        {
