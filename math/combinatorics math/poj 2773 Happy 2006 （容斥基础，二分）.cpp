...
//    cin>>t;
//    while(t--)
    while(scanf("%lld%lld",&n, &m) == 2)///问第几个互素的数，已知小于x有多少和n互素的数，二分求x使小于x有m个与n互素的数，然后暴力找到目标与n互素的x
    {
        getFactors(n);
        LL ans = binarySolve(1, 1000000000000, n, m);///注意素数的个数范围1e8，实际二分范围要更大，此处取1e12
