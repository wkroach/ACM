...
LL solve(LL n, LL mod)///计算小于等于n与n互素的数的4次方的和（取模），解法与hdu 1695类似，由于n的素因子数目不多，暴力容斥即可
{
    LL ans = 0;
    LL tmp3 = 0;
    for(LL i = 1; i < (1 << factors.size()); ++i)
    {
        ...
        if(cnt & 1)
        {
            ...
            ans += sumPow4(tmp3, mod) * quickMod(tmp2, 4, mod) % mod;///这些数均为x^4*tmp2^4这种形式，1<=x<=tmp3
        }
        else
        {
            tmp3 = n / tmp2;
            ans = ((ans - (sumPow4(tmp3, mod) * quickMod(tmp2, 4, mod) % mod)) + mod) % mod;///原理同上，注意减法取模的问题
        }
    }
    return ((sumPow4(n,mod) - ans) % mod + mod) % mod;///之前算的是不与n互质的数，因此要减去，注意减法取模的问题
}
